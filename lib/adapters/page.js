...
export function mapStrapiPageToFrontend(page, strapiUrl = '') {
  ...
  const textImageCommon = (block) => ({
    ...
    textBoxColor: block.textBoxColor ?? undefined,
  ...
  const components = (page.components || []).map((block) => {
    ...
    if (type === 'content-blocks.cta') {
      const titleHighlightWords = (block.titleHighlightWords ?? '')
        .split(',')
        .map((w) => w.trim())
        .filter(Boolean);
      return {
    ...
    if (type === 'content-blocks.text-image-block') {
      return {
        type: 'text-image',
        layout: 'side',
        ...textImageCommon(block),
    ...
    if (type === 'content-blocks.text-image-block-with-text-overlay') {
      return {
        type: 'text-image',
        layout: 'textOverlay',
        ...textImageCommon(block),
    ...
    if (type === 'content-blocks.timeline-block') {
      return {
        type: 'timeline',
  ...
  const heroHighlightWords = (page.heroHighlightWords ?? '')
    .split(',')
    .map((w) => w.trim())
    .filter(Boolean);
  ...
  return {
    ...
    heroImageUrl,
    heroVideoUrl,
    heroHeight: page.heroHeight ?? null,
    heroHighlightWords,
    components,
  };
}
