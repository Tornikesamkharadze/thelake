...
// Fixes image URLs that Strapi stored with localhost (e.g. during local dev/seeding).
export function fixStrapiUrl(url) {
  if (!url || typeof url !== 'string') return url;
  if (url.startsWith('http://localhost') || url.startsWith('https://localhost')) {
    return url.replace(/^https?:\/\/localhost:[0-9]+/, STRAPI_URL);
  }
...
async function request(path, params = {}) {
  ...
  if (params.populate !== undefined) {
    ...
    } else if (typeof params.populate === 'object' && params.populate !== null) {
      encodeBracketParams('populate', params.populate).forEach((pair) => {
  ...
  const res = await fetch(url, { next: { tags: ['strapi'] } });
...
export async function getGalleries(params = {}) {
  return request('galleries', { populate: { images: true }, ...params });
}
...
export async function getPageByPath(path, params = {}) {
  const { populate: _ignored, ...rest } = params;
...
/** @returns {Promise<{ data: object[], meta?: object }>} */
export async function getPartnerProjects(params = {}) {
  return request('partner-projects', { populate: '*', ...params });
}

/** @returns {Promise<{ data: object | null }>} Single type; 404 when no document for locale returns { data: null }. */
export async function getPartnersSection(params = {}) {
  try {
    return await request('partners-section', { populate: '*', ...params });
  } catch (err) {
    if (err.message?.includes('404')) {
      return { data: null };
    }
...
export async function getTimelineItems(params = {}) {
  return request('timeline-items', { populate: '*', sort: ['order:asc', 'date:asc'], ...params });
}
